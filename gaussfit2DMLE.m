...
function [A, x0, y0, sigma, offset,negL,variance] = gaussfit2DMLE(z, tolz, params0, px, py)
...
%residual = sum(sum((expected(px(:),py(:),params)-z(:)).^2));
...
%  Gaussian spatial distribution
    dr = [px(:)-params(2),py(:)-params(3)];
    gaussprob = params(5)*exp(-sum(dr.*dr,2)/2/params(4)/params(4)) + params(1);
    Lk = (z(:)).*log(gaussprob) - gaussprob;  % - z(:).*log(z(:))+model%log-likelihood; Poisson-distr.
    negL = -sum(Lk(:));
end
...
% The expected counts per pixel. (SOM Eq. 12)
    E = params (1)*twoDGauss (x,y,params (2),params (3),params (4))+params (5)^2;
end